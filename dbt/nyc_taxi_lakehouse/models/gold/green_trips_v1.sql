with trips as (
  select * from {{ ref('green_tripdata_silver') }}
),
zones as (
...
select
  ...
  cast(trips.ehail_fee as double) as ehail_fee,
  cast(trips.improvement_surcharge as double) as improvement_surcharge,
  cast(trips.total_amount as double) as total_amount,
  cast(trips.payment_type as double) as payment_type,
  cast(trips.trip_type as double) as trip_type,
